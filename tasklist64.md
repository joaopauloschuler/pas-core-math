...
- [X] **5.02** `pow`     — 1951 lines *(dint + qint + fenv, bivariate)*
  ...
  - **Sub-plan:**
    ...
    - [X] **5.02h** Added `PowQ2`, `PowP2`, `PowLog2`, `PowExp2` to `pow_port.inc` (dint slow path). **Sharp edges discovered**: (1) FPC `shr` on `Int64` is **logical** shift, not arithmetic; the `M := k >> 12` from C must be wrapped in `PowAsr12` helper to truncate-toward-minus-infinity for negative `k`. (2) The +1 ex offset from Pascal convention forces TWO algorithmic adjustments inside log_2: (a) the j-table shift is `51 - z.ex` (not `63-13-z.ex`), (b) `E` must be initialized to `X.ex - 1` (the C-binary exponent), not `X.ex`, so that `E*log(2)` and post-reduction xLoc both end up with the right magnitude. (3) PowExp2's threshold `xLoc.ex >= 11` corresponds to C `>= 10`, and the underflow/overflow ex values shift to `-1075`/`1026`. Cross-checked: log(2), log(10), log(0.5), exp(0), exp(1), exp(ln 2), exp(-1) all match DBL exactly; round-trip exp(log(17)) = 17 exactly.
    - [X] **5.02i** Added `PowQ3`, `PowP3`, `PowLog3`, `PowExp3` to `pow_port.inc` (qint third Ziv iteration). Added `QIntToI` to `pascoremathtypes.pas` (interface + implementation; mirrors C `qint_toi` directly since QInt convention here matches C — `QINT_ONE.ex=0`, no offset). PowQ3 unrolled into the three regimes from C: 64-bit Q_3[0] (mul_qint_11/add_qint_22), 128-bit Q_3[1..6] (mul_qint_22/add_qint_22), 256-bit Q_3[7..11] (mul_qint_33/add_qint), and full Q_3[12..14] (mul_qint/add_qint). PowP3 likewise mirrors p_3's four regimes (mul_qint_11, mul_qint_21, mul_qint_31, mul_qint_41) and ends with the polynomial×z multiplication. **Sharp edges**: (1) `K` collides with `k` under Pascal case-insensitivity → renamed to `Kq`/`kInt` like PowExp1. (2) C `for (k=2; k<7; ...)` style loops over individual scalar constants (`cPowQ3_0..14`, `cPowP3_0..17`) had to be unrolled — Pascal can't index named consts. (3) `2^12/log(2) = 5909.99…` so `QIntToI(QINT_LOG2_INV)` correctly returns 5909 (truncation toward zero). Cross-checked via `tmp/TestPow3`: exp(0)=1, exp(1)≈e, exp(ln 2)=2, exp(-1)≈1/e, log(2)/log(10)/log(0.5) all match DBL through high-limb display, exp(log(17))=17 exactly. Full `build.sh -dAVX2` clean; `TestQInt64` ALL OK; `TestHarness64 --pct 1` unchanged (only the pre-existing tgamma "FAIL max_ulp=0").
    - [X] **5.02j** Added `PowExtract`, `PowFastExtract`, `PowIsInt`, `PowPow2`, `PowRound54`, `PowExactPow`, `PowIsExact` to `pow_port.inc` (mirrors C `extract`, `fast_extract`, `is_int`, `pow2`, `round_54`, `exact_pow`, `is_exact` from `pow.h` / `pow.c`). Local `pcr_ctzll` helper added (just wraps `BsfQWord`). Sharp edges: (1) Identifier `G` collides with a global constant in `pascoremathhelperfuncs.pas` — renamed to `Glsb`. (2) Pascal +1 ex offset on TDInt64 means C `round_54.G_C = ex_C - 53` becomes `Glsb = ex_P - 54`, and the `d` constructed in exact_pow needs `d.ex_P = G + 64 - cnt` (vs C's `G + 63 - cnt`).
    - [X] **5.02k** Added `PcrPowPas` to `pow_port.inc` mirroring `cr_pow` (pow.c:1495). Implements the full special-case cascade (NaN/±Inf/±0/sign/|x|=1) plus the three Ziv phases. Helpers `PowQIntFromD`, `PowSubnormalizeQInt`, `PowQIntToD`, `PowIsSignaling` added. Sharp edges: (1) Pascal case-insensitivity again — local `X, Y, R: TDInt64` collided with args `x, y` (and `r` would too); renamed to `Xd, Yd, Rd_dint`. (2) Phase-2 rounding-test thresholds shift by +1 from C (Pascal `R.ex < -1074` ↔ C `< -1075`; Pascal `< -1021` ↔ C `< -1022`; subnormal-shift `ex = -(1021 + R.ex_P)` ↔ C `-(1022 + R.ex_C)`). (3) `not r1q = 0` parses wrong in Pascal — needs `(not r1q) = 0`.
    - [X] **5.02l** Wired `{$I pow_port.inc}` into `pascoremath64.pas` and switched dispatch from `cr_pow` to `PcrPowPas`. (`ccoremath64.pas` already exposed `cr_pow` for the test harness side-by-side, no change needed.)
    - [X] **5.02m** `TestHarness64 --pct 5 pow` reports 500,000 / 500,000 PASS (max_ulp=0). `Benchmark64 pow` shows Pascal 14.0 Mops/s vs C 22.5 Mops/s on 200M random bit-pattern inputs; sink mismatch is NaN-payload-only (XOR comparison; the harness uses ULP comparison and reports zero mismatches). Full `--pct 1` regression suite still 40 PASS / 1 FAIL with only the pre-existing tgamma failure.
