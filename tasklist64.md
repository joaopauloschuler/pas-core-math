...
- [ ] **2.07** `atan2`   — 586 lines  *(**TInt64 (192-bit)** + dd + fenv, bivariate — NOT pure dd)*
  - **Dependency satisfied (2026-04-25):** `tint.h` ported as task 2.07a above; `TInt64` arithmetic available in `pascoremathtypes.pas`. Next steps: port atan2.c (586 lines) using the new infrastructure, including the `fenv_t`/`feholdexcept`/`fetestexcept`/`feupdateenv` exception tracking for underflow/overflow around the `y/x` quotient. Tables (P[30], Q[30] tint_t rationals) are shared with 2.11 `atan2pi`.
